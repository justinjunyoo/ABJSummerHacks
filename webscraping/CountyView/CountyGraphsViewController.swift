...
class CountyGraphsViewController: UIViewController {
    ...
    let countyName = UserDefaults.standard.string(forKey: "name")
    
    override func viewDidLoad() {

        topnavigationBar.setValue(true, forKey: "hidesShadow")
        
        segment.backgroundColor = AppDelegate().uicolorFromHex(rgbValue: 0x313547)
        segment.setTitleTextAttributes([.foregroundColor: UIColor.lightText], for: .normal)
        segment.setTitleTextAttributes([.foregroundColor: UIColor.lightText], for: .selected)
        segment.isHighlighted = true
        
        
        
        navigationBar.topItem?.title = countyName
